...
# CompleteAndGrounded — LLM-based combined metric for completeness and groundedness
...
# Colors — color-related reasoning dataset
from dspy.datasets import Colors

# AlfWorld — interactive text-based environment dataset
from dspy.datasets import AlfWorld
...
# Universal data loading with DataLoader
from dspy.datasets import DataLoader
dl = DataLoader()

# From HuggingFace Hub
dataset = dl.from_huggingface("squad", fields=["question", "context", "answers"], input_keys=("question", "context"), split="train")

# From local files
dataset = dl.from_csv("data.csv", fields=["question", "answer"], input_keys=("question",))
dataset = dl.from_json("data.json", fields=["question", "answer"], input_keys=("question",))
dataset = dl.from_pandas(df, fields=["question", "answer"], input_keys=("question",))
dataset = dl.from_parquet("data.parquet", fields=["question", "answer"], input_keys=("question",))
```

---

## Output Quality Enforcement (dspy.Refine / dspy.BestOfN)

**DEPRECATED:** `dspy.Assert` and `dspy.Suggest` have been removed in DSPy 3.1.x. Use `dspy.Refine` or `dspy.BestOfN` instead.

### dspy.Refine — iterative refinement with feedback

Each attempt receives automatic feedback from the reward function about why the previous attempt failed, so the model can improve iteratively.

```python
def quality_metric(args_dict, prediction):
    """Reward function: (args_dict, prediction) -> float"""
    has_sources = len(prediction.sources) >= 1
    is_substantive = len(prediction.answer.split()) >= 5
    return float(has_sources and is_substantive)

refine = dspy.Refine(
    module=dspy.ChainOfThought("question -> answer, sources: list[str]"),
    N=3,                       # max attempts
    reward_fn=quality_metric,  # (args_dict, prediction) -> float
    threshold=1.0,             # score needed to accept
)
result = refine(question="What causes tides?")
```

### dspy.BestOfN — independent sampling, pick best

Runs N independent samples and returns the one with the highest reward score. No feedback between attempts.

```python
def my_metric(args_dict, prediction):
    """Reward function: (args_dict, prediction) -> float"""
    return float(len(prediction.answer.split()) >= 10)
...
- **dspy.Refine**: Each attempt learns from previous failures via automatic feedback. Better when the model can correct itself given specific error information. Uses fewer total LM calls when early attempts succeed.
- **dspy.BestOfN**: All N attempts run independently (no feedback). Better when you want diverse outputs and want to pick the best one. All N calls happen regardless of quality.
