...
```python
# For reasoning models that require "developer" role instead of "system":
lm = dspy.LM('openai/o3-mini', model_type='responses', use_developer_role=True)
```

---

## Experimental Types (3.1.0+)
...
# Create documents
docs = [
    Document(data="Paris is the capital of France.", title="Geography"),
    Document(data="The Eiffel Tower is in Paris.", title="Landmarks"),
]
# Citations are auto-adapted for Anthropic models with native citation support
```
...
### `dspy.asyncify()` / `dspy.syncify()`

```python
# Convert sync module to async-compatible
async_module = dspy.asyncify(my_sync_module)
result = await async_module(question="...")

# Convert async module to sync
sync_module = dspy.syncify(my_async_module, in_place=True)  # patches forward to call aforward
# or
sync_module = dspy.syncify(my_async_module, in_place=False)  # wraps in SyncWrapper
...
```python
class FullCallback(BaseCallback):
    # LM hooks
    def on_lm_start(self, call_id, instance, inputs): ...
    def on_lm_end(self, call_id, outputs, exception): ...
...
dspy_tool = convert_langchain_tool(langchain_tool)
...
# Or: search = dspy.retrievers.Embeddings(embedder=embedder, corpus=[])
...
#### CodeInterpreter protocol (3.1.3+)

```python
# New in 3.1.3: CodeInterpreter protocol
from dspy import CodeInterpreter, FinalOutput, CodeInterpreterError
# CodeInterpreter is the protocol for sandboxed execution environments (used by RLM, ProgramOfThought)
# FinalOutput signals completion from code interpreter
# CodeInterpreterError wraps execution failures
```
...
# Context manager form (alternative to dspy.configure(track_usage=True))
...
# Clone an LM with modified params
fast_lm = lm.copy(temperature=0.0, max_tokens=100)

# rollout_id — differentiates cache entries for diverse generation
result = module(question="...", config={"rollout_id": 1})

# Save settings (cloudpickle-based)
dspy.settings.save("dspy_settings.pkl")

# Load settings (3.1.1+)
config = dspy.load_settings("dspy_settings.pkl")  # returns dict

# Configure cache explicitly
dspy.configure_cache(
    enable_disk_cache=True,
    enable_memory_cache=True,
    disk_cache_dir="/path/to/cache",       # custom cache directory
    disk_size_limit_bytes=2_000_000_000,   # 2GB disk cache limit
    memory_max_entries=10_000,              # max in-memory cache entries
)
# Environment variables: DSPY_CACHEDIR, DSPY_CACHE_LIMIT

# Pydantic-style constraints on fields (auto-translated to human-readable in prompts)
class StrictSig(dspy.Signature):
    count: int = dspy.OutputField(gt=0, le=100, desc="a count between 1 and 100")
    name: str = dspy.OutputField(min_length=2, max_length=50)
```
